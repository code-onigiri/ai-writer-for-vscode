...
- [ ] 1.1 pnpmワークスペースとパッケージ構成をセットアップする
...
- [ ] 3.3 Gemini CLIアダプターを実装する
  - child_processでCLIを起動し、JSONモード出力を正規化してAISDK Hubに返す。
  - CLIエラーコードの再分類と再試行ハンドリングを加える。
  - CLI設定の有効性検証とユーザーフィードバックをBase層に返す。
  - _Requirements: R5_
